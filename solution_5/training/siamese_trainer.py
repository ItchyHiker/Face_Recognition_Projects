...
from loss import OHEMCrossEntropyLoss, FocalCrossEntropyLoss, ContrastiveLoss
        ...
        for batch_idx, sample in enumerate(self.dataloaders[phase]):
            ...
            if batch_idx % 40 == 0:
                print('Train Epoch: {} [{:08d}/{:08d} ({:02.0f}%)]\tLoss:{:.6f}\tAcc:{:.6f} LR:{:.7f}'.format(
                    ...
                    100. * batch_idx / len(self.dataloaders[phase]), loss.item(), 0, self.optimizer.param_groups[0]['lr']))
